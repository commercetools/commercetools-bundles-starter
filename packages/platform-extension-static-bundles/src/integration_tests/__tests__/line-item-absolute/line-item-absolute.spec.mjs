...
import { Carts } from '../../shared-fixtures/index.mjs';
import {
  ...
  updateResource,
  TEST_TIMEOUT,
  getLineItemBySku, createCTClient,
} from '../../test-utils.mjs';
...
describe('Line-item-absolute', () => {
  ...
  describe('When creating a cart (1x$55.00, 2x$50.00 =$155) with line items and applying a 10off absolute discount, not ignoring quantity, no exclusions', () => {
    it('Cart should be discounted by $10 after applying the $10 off discount', async () => {
      const expectedTotalPriceDifference = cartDiscountToCreate.custom.fields.money.centAmount;

      const expectedDiscountId = createdDiscount.id;
      const createdCart = await ensureResourcesExist(ctClient, cartToCreate, 'carts');
      const actions = [
        {
      ...
      const receivedTotalPriceDifference = createdCart.totalPrice.centAmount
...
describe('When creating a cart with x2 of a $189 item and 2 discounts', () => {
  ...
  after(async () => {
    if (createdCartDiscountBlazer.id) {
      await deleteResources(ctClient, createdCartDiscountBlazer, 'cartDiscounts');
    }
  ...
  it('Cart should _not_ be discounted after removing a line item', () => {
    const createdCart = ensureResourcesExist(ctClient, cartForThings, 'carts');
    createdCartDiscountBlazer = ensureResourcesExist(
      ctClient,
      cartDiscountBlazer,
      'cartDiscounts',
    );
    createdCartDiscountTierTwo = ensureResourcesExist(
      ctClient,
      cartDiscountTierTwo,
      'cartDiscounts',
    );
    ...
    const updatedCart = updateResource({
      resource: createdCart,
      resourceTypeId: 'carts',
      actions,
    });
